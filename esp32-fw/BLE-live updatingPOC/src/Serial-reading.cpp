...
#define CHARACTERISTIC_UUID_TX2 "f2ec7a09-c4a9-4c22-a4ed-25a37ef9bbd5"

BLEServer* pServer = NULL;
BLECharacteristic* pRxCharacteristic = NULL;
BLECharacteristic* pTxCharacteristic = NULL;
BLECharacteristic* pTx2Characteristic = NULL;
...
void calibrate() {
    ...
    bl0937.expectedActivePower(60.0);
    bl0937.expectedVoltage(230.0);
    bl0937.expectedCurrent(60.0 / 230.0);

    // Show corrected factors
#ifdef SERIAL_EN
    Serial.print("[HLW] New current multiplier : "); Serial.println(bl0937.getCurrentMultiplier());
    Serial.print("[HLW] New voltage multiplier : "); Serial.println(bl0937.getVoltageMultiplier());
    Serial.print("[HLW] New power multiplier   : "); Serial.println(bl0937.getPowerMultiplier());
#endif
}
...
void setup() {
    ...
    pTx2Characteristic = pService->createCharacteristic(
...
void loop() {
    ...
    if ((millis() - last) > UPDATE_TIME) {
        ...
        apppwr = bl0937.getApparentPower();
        pfactor = (100 * bl0937.getPowerFactor());
        energy = bl0937.getEnergy();
        std::string current2 = String(current, 2 ).c_str();
        std::string voltage2 = String(voltage).c_str();

        if(deviceConnected) {
        pTxCharacteristic->setValue(voltage2);
        pTx2Characteristic->setValue(current2);
        }
